...
const CHINA = () => {
    ...
    const intitializeMovies = useCallback(async({
        ...
        const fetchMoviesFromAPI = async (actual_index) => {
            ...
            const temp_movies = [
                { index: "china movie", results: [], api: "discover/movie", page: 1, total_pages: 0, type:"movie" },
                { index: "china tv", results: [], api: "discover/tv", page: 1, total_pages: 0,type:"tv" },
            ];
            ...
            async function freshFetch(){
                ...
                const response = await fetch(`${safeKeys.MOVIE_DB}${temp_movies[key].api}?api_key=${safeKeys.API_KEY}&with_original_language=zh&page=${temp_movies[key].page}`);
                ...
                async function freshFetch(){
                    ...
                    const insertContent = (await Promise.all(runContent.map(async actual_index => {
                        ...
                        const response = await fetch(
                            `${safeKeys.MOVIE_DB}${temp_movies[key].api}?api_key=${safeKeys.API_KEY}&with_original_language=zh&page=${temp_movies[key].page}`
                        );
                        const data = await response.json();
        ...
        <div className="w-[100%] duration-250 h-[100%] text-white flex flex-row flex-wrap" style={{background:"linear-gradient(65deg, #0d0d0d, rgba(0,0,0,0.75), #1c2a3b, #0f111a)"}}>
            ...
            <div className={windowWidth > 800 ? "w-[80%] component-wall movie-scene h-[100%] ml-[20%] overflow-y-auto flex flex-col":"w-[100%] movie-scene overflow-y-auto h-[92%] flex flex-col"}>
                ...
                <div className="w-[100%] h-auto">
                    <LATEST api={"&with_original_language=zh"} wireframe={"china"}/>
                </div>
                {
