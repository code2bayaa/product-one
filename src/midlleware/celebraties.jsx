...
const CELEBRATIES = ({actedMovies, mode}) => {
    ...
    //         console.log(hashedKey,"first hashed")
    //         console.log("initialize",mode,hashed) 
    ...
    //                         const response = await fetch(`${safeKeys.MOVIE_DB}${api}?api_key=${safeKeys.API_KEY}&language=en-US`)
    //                         const {cast = [],crew = []} = await response.json()
                            
    //                         if(jobId === "Actor"){
    //                             // Take top 2 actors by 'order'
    ...
    //                     setThemes(() => [...peopleArray[0]].sort((a,b) => b.popularity - a.popularity).slice(0,20))
    ...
    //         // console.log(hashedKey,page,"hashed")
                ...
                if (actedMovies.length > 0) {
                    ...
                    let peopleArray = await Promise.all(actedMovies.map(async({results}) => {
                        ...
                        return (await Promise.all(results.map(async({id}) => {
                            ...
                            let api = mode === "tv" ? `tv/${id}/credits` : `movie/${id}/credits`
                            try{
                                const response = await fetch(`${safeKeys.MOVIE_DB}${api}?api_key=${safeKeys.API_KEY}&language=en-US`, { signal })
                                if (!response.ok) {
