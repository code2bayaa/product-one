...
const NETFLIX = () => {
    ...
    const intitializeMovies = useCallback(async({
        ...
        const fetchMoviesFromAPI = async (actual_index) => {
            ...
            const temp_movies = [
                { index: "discover netflix movie", results: [], api: "discover/movie", page: 1, total_pages: 0 },
                { index: "discover netflix tv", results: [], api: "discover/tv", page: 1, total_pages: 0 },
                // { index: "discover netflix person", results: [], api: "discover/person", page: 1, total_pages: 0 },
            ];
    ...
    const intitializePeople = useCallback(async({
        ...
        const fetchPersonFromAPI = async (actual_index) => {
            ...
            const current_date = new Date().toISOString().split("T")[0]
            const temp_people = [
                {"index":"discover netflix person","results":[],"api":"discover/person","page":1,people_page:1,total_pages:0,total_results:0},
            ]
            ...
            async function freshFetch(){
                // Fetch data from the API if not found in the cache
                const response = await fetch(
                    `${process.env.REACT_APP_movie_db}${temp_people[key].api}?api_key=${process.env.REACT_APP_api_key}&language=en-US&page=${temp_people[key].page}&with_watch_providers=8&watch_region=US`
                );
                const data = await response.json();
