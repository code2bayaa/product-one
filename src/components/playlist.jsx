...
const PLAYLIST = () => {
    ...
    //                 let apiUrl; 
    //                 if(type === "movie") {
    //                     apiUrl = `${safeKeys.MOVIE_DB}movie/${playlistData.tag}?api_key=${safeKeys.API_KEY}`
    //                 }else if(type === "tv") {
    //                     apiUrl = `${safeKeys.MOVIE_DB}tv/${playlistData.tag}?api_key=${safeKeys.API_KEY}`
    //                 }else if(type === "season") {
    //                     apiUrl = `${safeKeys.MOVIE_DB}tv/${playlistData.tag}/season/${season}?api_key=${safeKeys.API_KEY}`
    //                 }else if(type === "episode") {
    //                     apiUrl = `${safeKeys.MOVIE_DB}tv/${playlistData.tag}/season/${season}/episode/${episode}?api_key=${safeKeys.API_KEY}`   
    //                 }
        ...
        <div className="w-[100%] duration-250 h-[100%] text-white flex flex-row flex-wrap" style={{background:"linear-gradient(65deg, #0d0d0d, rgba(0,0,0,0.75), #1c2a3b, #0f111a)"}}>
            ...
            <div className={windowWidth > 800 ? "w-[80%] h-[100%] overflow-y-auto movie-scene ml-[20%] flex flex-col":"w-[100%] h-[92%] overflow-y-auto movie-scene flex flex-col"}>
...

                {
                                ...
                                <div className={`w-[100%] duration-50 movie-scene ${windowWidth > 800 ? "h-[400px]" : "h-[300px]"} flex flex-col flex-wrap overflow-x-auto overflow-y-hidden my-[1%]`}>
                                    {
                                        result.map(({season_number,episode_number,poster_path,still_path,backdrop_path,vote_average,popularity,vote_count,name,original_name,title,original_title,id},movie_key) => 
                                            <div 
                                                key={movie_key} 
                                                onClick={() => navRoute({
                                                    url:type === "tv" ? `/playlist/serie` : key === "movie" ? `/playlist/movie` : key === "season" ? `/playlist/season` : `/playlist/episode`,
                                                    state:{
                                                        id,
                                                        // episodeID:value.tags[movie_key].tag,
                                                        // seasonID:value.tags[movie_key].tag,
                    ...
                    :
                        <img src="/image/playlist.svg" width={200} height={200} className="w-[80%] ml-[10%] h-[80%]" alt="late developers https://wekesa-apps.io" />
