...
const NETFLIX = () => {
    ...
    const intitializeMovies = useCallback(async({
        ...
        const fetchMoviesFromAPI = async (actual_index) => {
            ...
            const temp_movies = [
                { index: "discover netflix movie", results: [], api: "discover/movie", page: 1, total_pages: 0, type:"movie" },
                { index: "discover netflix tv", results: [], api: "discover/tv", page: 1, total_pages: 0,type:"tv" },
                            ...
                            <div className={`w-[100%] duration-50 movie-scene ${windowWidth > 800 ? "h-[400px]" : "h-[200px]"} flex flex-col flex-wrap overflow-x-auto overflow-y-hidden my-[1%]`}>
                                {
                                    results.map(({adult,backdrop_path,genre_ids,id,original_language,original_name,name,original_title,overview,popularity,poster_path,release_date,title,video,vote_average,vote_count},movie_key) => 
                                        <div 
                                            key={movie_key} 
                                            onClick={() => navMovie(id,name || original_name ? `/netflix/serie` : `/netflix/movie`)}
                                            className={windowWidth > 800 ? "cursor-pointer w-[25%] h-[100%] hover:contrast-150":`${index === "popular" || index === "airing" ? "w-[50%]" :"w-[45%]"} h-[100%] hover:contrast-150`}
                                        >
                                            <div 
