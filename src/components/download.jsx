    ...
    return (
        <div className="w-[100%] duration-250 h-[100%] text-white" style={{background:"linear-gradient(65deg, #0d0d0d, rgba(0,0,0,0.75), #1c2a3b, #0f111a)"}}>
            <div className={windowWidth > 800 ? "w-[90%] h-[100%] flex flex-wrap flex-col mx-[5%]" : "w-[100%] h-[100%] flex flex-wrap flex-col mt-[10%]"}>
                <h1 className="my-t-[5%]">YOUR OFFLINE DOWNLOADS</h1>
                ...
                <div className={`w-[100%] h-[90%] overflow-y-auto flex flex-row flex-wrap`}>
                    {console.log(movies)}
                    {
                        movies && movies.map(({
                            ...
                            >
                                    <PICTURE key={data?.id} classes={`object-cover h-[100%] w-[60%] ${windowWidth > 800 ? "" : "rounded-xl"}`} url={true} picture={(image && URL.createObjectURL(image)) || '/image/logo.png'} />
                                        ...
                                        <button
                                            onClick={() => {
                                                Swal.fire({
                                                    ...
                                                    text: 'This will delete the saved video permanently.',
                                                    icon: 'warning',
                                                ...
                                                .then(async({isConfirmed}) => {
                                                    if (isConfirmed) {
                                                        await remove(name);
                                                        await Swal.fire('Removed', 'Download removed', 'success');
                                                    }
                                                })
